...
function DeleteButton({ commentId, getData }) {
    async function handleDelete(getCommnetId) {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/comment/delete/${getCommnetId}`, {
                method: "DELETE",
                headers: {
    ...
    return <AlertDialog>
        ...
        <AlertDialogContent>
            ...
            <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={() => handleDelete(commentId)} >Continue</AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
    </AlertDialog>
}
...
const CommentsTableRow = ({ srNo, commentItem, getData }) => {
    ...
    return (
        <TableRow>
            ...
            <TableCell className={'line-clamp-1'}>{commentItem.blog?.title}</TableCell>
            <TableCell>{commentItem.user.name}</TableCell>
            <TableCell>{commentItem.comment}</TableCell>
            <TableCell>{new Date(commentItem.createdAt).toLocaleDateString()}</TableCell>
