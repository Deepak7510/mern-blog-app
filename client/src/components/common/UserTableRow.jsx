  ...
  async function handleDelete(getUserId) {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/delete/${getUserId}`,
        {
          method: "DELETE",
          headers: {
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. It will permanently remove this user
...
const UsersTableRow = ({ srNo, userItems, getData }) => {
  return (
    <TableRow>
      ...
      <TableCell>
        {new Date(userItems.createdAt).toLocaleDateString()}
      </TableCell>
