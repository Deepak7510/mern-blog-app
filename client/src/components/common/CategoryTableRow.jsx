...
function DeleteButton({ categoryId, getData }) {
    async function handleDelete(getCategoryId) {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/category/delete/${getCategoryId}`, {
                method: "DELETE",
                headers: {
    ...
    return <AlertDialog>
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    This action cannot be undone. It will permanently remove this category and its related data.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={() => handleDelete(categoryId)} >Continue</AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
    </AlertDialog>
}
    ...
    return (
        <TableRow>
            ...
            <TableCell className={'space-x-2'}>
                <Button onClick={() => handleEdit(categoryData)}><Edit /></Button>
