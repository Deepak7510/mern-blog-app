...
const LikeCount = ({ blogId }) => {
  ...
  const { loading, data: likeCountData } = useFetch(
    `${import.meta.env.VITE_BACKEND_URL}/api/like/count/${blogId}`,
    {},
    []
  );
  ...
  const { data: likeStatusData } = useFetch(
    `${import.meta.env.VITE_BACKEND_URL}/api/like/check/${user?._id || ""}/${
      blogId || ""
    }`,
    {
      method: "GET",
  ...
  async function handleToogleLike() {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/like/toggle`,
        {
          method: "POST",
          headers: {
            "Content-type": "application/json",
