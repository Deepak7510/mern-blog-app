...
const BlogByCategory = () => {
    ...
    const { data, loading } = useFetch(`${import.meta.env.VITE_BACKEND_URL}/api/blog/category/${category}`, { method: "GET", headers: { 'Content-Type': 'application/json' } }, [category]);
    return (
        <div className='m-2 md:m-3'>
            {
                loading && data == null ?
                    ...
                    :
                    <h1 className='text-2xl font-medium m-2 md:m-3 pb-2 border-b'>{data?.category}</h1>
            }
            <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-3'>
                {loading ?
                    ...
                    :
                    data && data.categoryBlogList.length > 0 ? data?.categoryBlogList.map(item => {
