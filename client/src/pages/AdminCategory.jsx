...
const AdminCategory = () => {
    ...
    const { getData, data, loading } = useFetch(`${import.meta.env.VITE_BACKEND_URL}/api/category/fetch`, { method: "GET", headers: { 'Content-Type': 'application/json' }, credentials: 'include' }, []);
    const form = useForm({
    ...
    return (
        <Card className='m-2 md:m-3'>
            <CardHeader>
                <Button className={'w-fit'} onClick={() => setOpenCategoryDialog(true)}>Add Category</Button>
                <Dialog open={openCategoryDialog} onOpenChange={resetAll} >
                    <DialogContent aria-describedby={undefined}>
                        {form.formState.isSubmitting ? <Loading2 /> : null}
                        <DialogHeader>
                            <DialogTitle className='font-extrabold'>{editId ? 'Edit Category' : 'Add Category'}</DialogTitle>
                            <Form {...form}>
                    ...
                    <TableBody>
                        {loading ?
                            ...
                            : (data && data?.length > 0 ? data.map((item, index) => (
                                <CategoryTableRow srNo={index} key={item._id} handleEdit={handleEdit} getData={getData} categoryData={item} />
                            )) :
                            <TableRow>
