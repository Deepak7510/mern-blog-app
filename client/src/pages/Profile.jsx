...
const Profile = () => {
  ...
  const { loading, data } = useFetch(`${import.meta.env.VITE_BACKEND_URL}/api/user/profile/${userData._id}`, {
    method: 'GET',
    headers: {
  ...
  async function onSubmit(data) {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/user/update/${userData._id}`, {
        method: 'PUT',
        body: formData,
      });
    ...
    <div className=' grid lg:grid-cols-2 gap-4 m-2 md:m-3'>
      {loading?
        ...
        <Card className={'w-full h-fit sticky top-0'}>
          ...
          <CardHeader className={'flex justify-center items-center w-full'}>
            <Label htmlFor='profile' onDragOver={handleDragOver} onDrop={handleDrop}   >
              <Avatar className={'h-22 w-22 lg:h-32 lg:w-32 group'}>
                {
                  preview ?
                    ...
                    : data && data.avatar ?
                      ...
                      :
                      <AvatarFallback className={'text-5xl font-bold'}>{data?.name[0]}</AvatarFallback>
                }
                <div className='absolute top-[50%] left-[50%] z-10 transform translate-[-50%]'><CameraIcon className='h-20 w-10 opacity-0 group-hover:opacity-40' /></div>
              </Avatar>
              <Input id='profile' onChange={(e) => { form.setValue("avatar", e.target.files[0]) }} className={'hidden'} type={'file'} />
            </Label>
          </CardHeader>
          <CardContent>
            <Form {...form}>
      ...
      <Card className={'w-full sticky top-'}>
